...
export function VotePhase() {
  const { state } = useGame();
  return state.mode === "3p" ? <ThreePlayerVote /> : <TwoPlayerVote />;
}
...
function ThreePlayerVote() {
  ...
  async function finishVoting(voteMap: Partial<Record<PlayerId, PlayerId>>) {
    setSubmitting(true);
    ...
    const votedOut = top[Math.floor(Math.random() * top.length)][0] as PlayerId;
...
function TwoPlayerVote() {
  ...
  async function finishGuessing(
    ...
    const winner: "humans" | "fail" =
      h1Correct && h2Correct ? "humans" : "fail";
  ...
  return (
    <Card className="flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        <Button onClick={() => castAndAdvance("same")} fullWidth>
          同じワード
        </Button>
        <Button
          variant="secondary"
